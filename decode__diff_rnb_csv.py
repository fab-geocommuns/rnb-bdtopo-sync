...
def tri_rnb_to_remove_or_to_calculate(batiments_rnb_last_changes):
    ...
    to_remove = set()
    to_calculate = set()
    batiments_rnb_last_changes_filtred = batiments_rnb_last_changes
    ...
    for batiment_rnb in batiments_rnb_last_changes:
        if (batiment_rnb['is_active']!="1"):
            ...
            batiments_rnb_last_changes_filtred.remove(batiment_rnb) #on retire le bati de la liste filtree
            continue
    ...
    return batiments_rnb_last_changes_filtred,to_remove,to_calculate
...
if not getDiff_RNB_from_date("2025-07-15T00:00:00Z",csv_diff_rnb):
...
rnb_to_remove, rnb_to_calculate = tri_rnb_to_remove_or_to_calculate(batiments_rnb_last_changes)
...
with open(csv_rnb_to_remove, mode='w', newline='', encoding='utf-8') as csv_file:
    writer = csv.writer(csv_file)
    writer.writerow(["rnb_id"])
    for id in rnb_to_remove:
        writer.writerow([id])

with open(csv_rnb_to_calculate, mode='w', newline='', encoding='utf-8') as csv_file:
    writer = csv.writer(csv_file)
    writer.writerow(["rnb_id"])
    for id in rnb_to_calculate:
        writer.writerow([id])
